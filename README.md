...
- `results.csv`: organic results with ads dropped, sorted by rating then review count. Columns are url, name, rating, review_count, review_count_is_exact, price, distance_miles, categories. Places with no reviews yet have an empty rating and count and sort to the bottom. `distance_miles` is the straight-line distance from the search center to the business, to one decimal, from the coordinates Yelp's own map call returns for each business
...
Yelp is behind DataDome, which blocks curl, headless Chrome, and Chrome started with Playwright's default automation flags. The script launches a visible Chrome through Playwright without those flags and with its own persistent profile under `~/.cache/best-restaurants/`, loads the search page once so the bot check clears, then fetches the remaining pages from inside that page with `fetch()` four to seven seconds apart. A profile Yelp has blocked gets wiped and the run retries once. Results come from the server-rendered HTML cards because the `/search/snippet` JSON the notebook used no longer includes organic businesses. Review counts above 1000 show up abbreviated (`1.9k`) on cards, so those businesses get one extra request each to read the exact count from the business page title

Yelp only prints a distance on a card when the location is a street address and it picked the search area itself, and it hides them as soon as a bounding box is passed, so the script computes them instead. Each search page carries a business id per card in its embedded JSON, and the map on the page loads coordinates for those ids through a `GetMapData` GraphQL call to `/gql/batch`, which takes a whole batch of ids at once. The script sends the ids it collected through that same call from inside the page, fifty at a time, and computes the great-circle distance from the search center. A batch that comes back without data is split in half and tried again, and a card that still ends up without coordinates keeps an empty distance. The call is a persisted query, so its id is read from the request the page itself makes and the script falls back to the last known one
