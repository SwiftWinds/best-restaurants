...
Flags: `--what` changes the search term (default Restaurants), `--radius` sets how many miles around the location to search (default 5, and 0 lets Yelp pick its own smaller area, about 2.7 miles for a Daly City address), `--out-dir` picks another output folder, `--channel` picks chrome, msedge, or chromium, `--rebuild` skips Yelp and rebuilds from a saved `search_pages.json`
...
The 240 rows are Yelp's most relevant matches inside the box re-sorted by rating, not the 240 highest rated places in the box, so a well-rated spot can be missing at any radius. Sofra Grill in South San Francisco is 4.4 miles from the Daly City address and shows up under `--what Turkish` but not under Restaurants. Sort order on Yelp's side is rating, and the script re-sorts by rating then review count to match the notebook

## Results

Both folders were produced with the default 5 mile radius
